...
// Function to create "results" folder if not exists
void create_results_folder() {
    struct stat st = {0};
    if (stat("results", &st) == -1) {
        mkdir("results");
        printf("Created folder: results\n");
    }
}
...
int* Hysteresis(int *cany_matrix, int h_thres, int l_thres, int img_h, int img_w);
...
//Y = 0.299R + 0.587G + 0.114B this is a complicated formula
//Greyscale function stores the coloured pixels in a different grey_matrix based on their colour intensities
// img_data[3*i]     → Red component
// img_data[3*i + 1] → Green component
// img_data[3*i + 2] → Blue component
...
//Applies gaussian blur to greyscale data for further processing
int* GaussianBlur(unsigned char* grey_matrix, int img_h, int img_w){
...
int* Hysteresis(int *canny_matrix, int h_thres, int l_thres, int img_h, int img_w){
	int img_size = img_h*img_w;
...
int* Sobble(int* blur_matrix, int img_h, int img_w){
	...
	int* hys_matrix = Hysteresis(canny_matrix,100,50, img_h,img_w );
	return hys_matrix;
}
...
int Image_Process(char src[50]){
	...
	//Finding if it has a colour table
	//If BITDEPTH <= 8 colour table exists
	...
	if(bitDepth <= 8){
		...
		int *blur_matrix = GaussianBlur(img_data, img_h, img_w);
	...
	//BITDEPTH = 24 => Colour
		...
		//Applying gaussian blur to greyscale image
		int *blur_matrix = GaussianBlur(grey_data, img_h, img_w);

		//Applying sobble operator to blurred image
		int *sobble_matrix = Sobble(blur_matrix, img_h, img_w);
